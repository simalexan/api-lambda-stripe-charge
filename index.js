...
exports.handler = (event) => {
  ...
  const chargeRequest = qs.parse(event.body);
  ...
  return ssm.getParameter({ Name: STRIPE_SECRET_KEY_NAME, WithDecryption: true }).promise()
    .then(response => {
      ...
      return chargeCustomer(stripeSecretKeyValue, chargeRequest.amount, chargeRequest.currency, chargeRequest.description);
    })
